...
from typing import Optional, Iterable, Sequence, Union, Literal, Dict, Tuple

import numpy as np
import pandas as pd
import scipy.sparse
from scipy.stats import pearsonr, spearmanr
from sklearn.metrics import mean_squared_error, auc, precision_recall_curve, roc_curve
...
def compute_wasserstein(pred: np.ndarray, true: np.ndarray, *_args, epsilon: float = 0.1) -> float:
...
def mmd_distance(x: np.ndarray, y: np.ndarray, gamma: float) -> float:
    ...
    xx, xy, yy = rbf_kernel(x, x, gamma), rbf_kernel(x, y, gamma), rbf_kernel(y, y, gamma)
    return xx.mean() + yy.mean() - 2 * xy.mean()


def compute_mmd(pred: np.ndarray, true: np.ndarray, *_args, gammas: Optional[Sequence[float]] = None) -> float:
...
def compute_pearson_delta(pred: np.ndarray, true: np.ndarray, ctrl: np.ndarray, *_args) -> float:
    """Pearson between mean differences from control."""
    return float(pearsonr(pred.mean(0) - ctrl.mean(0), true.mean(0) - ctrl.mean(0))[0])


def compute_pearson_delta_separate_controls(pred: np.ndarray, true: np.ndarray, ctrl: np.ndarray, pred_ctrl: np.ndarray) -> float:
    """Pearson between pred vs pred_ctrl and true vs ctrl deltas."""
    return float(pearsonr(pred.mean(0) - pred_ctrl.mean(0), true.mean(0) - ctrl.mean(0))[0])


def compute_pearson_delta_batched(batched_means: Dict[str, np.ndarray], *_args) -> float:
    """Pearson on aggregated deltas stored in dict."""
    pred_de = pd.DataFrame(batched_means["pert_pred"] - batched_means["ctrl_pred"]).mean(0)
    true_de = pd.DataFrame(batched_means["pert_true"] - batched_means["ctrl_true"]).mean(0)
...
def compute_gene_overlap_pert(pert_pred: np.ndarray, pert_true: np.ndarray, ctrl_true: np.ndarray, ctrl_pred: np.ndarray, k: int = 50) -> float:
...
def compute_gene_overlap_cross_pert(DE_true: pd.DataFrame, DE_pred: pd.DataFrame, control_pert: str = 'non-targeting', k: Optional[int] = None, topk: Optional[int] = None) -> Dict[str, float]:
    ...
    for pert in DE_pred.index:
        ...
        overlaps[pert] = len(set(true_subset) & set(pred_subset)) / denom if denom else 0.0
    return overlaps


def compute_sig_gene_counts(DE_true_sig: pd.DataFrame, DE_pred_sig: pd.DataFrame, pert_list: Sequence) -> Tuple[Dict[str, int], Dict[str, int]]:
    """Counts of significant DE genes per perturbation."""
    true_counts = {p: int(DE_true_sig.loc[p].dropna().shape[0]) if p in DE_true_sig.index else 0 for p in pert_list}
    pred_counts = {p: int(DE_pred_sig.loc[p].dropna().shape[0]) if p in DE_pred_sig.index else 0 for p in pert_list}
    return true_counts, pred_counts


def compute_sig_gene_spearman(true_counts: Dict[str, int], pred_counts: Dict[str, int], pert_list: Sequence) -> float:
...
class ClusteringAgreementEvaluator:
    ...
    @staticmethod
    def _score(labels_true: Sequence[int], labels_pred: Sequence[int], metric: Literal['ami', 'nmi', 'ari']) -> float:
        if metric == 'ami':
    ...
    def _cluster_leiden(
        ...
        if 'neighbors' not in adata.uns:
            sc.pp.neighbors(adata, n_neighbors=min(n_neighbors, adata.n_obs - 1), use_rep='X')
    ...
    def compute(
        ...
        self._cluster_leiden(ad_real_cent, self.true_resolution, real_key, self.n_neighbors)
...
def compute_directionality_agreement(DE_true_df: pd.DataFrame, DE_pred_df: pd.DataFrame, pert_list: Sequence, p_val_thresh: float = 0.05) -> Dict[str, float]:
    """Sign direction agreement for overlapping DE genes."""
    matches: Dict[str, float] = {}
    for p in pert_list:
        t = DE_true_df[(DE_true_df['target']==p)&(DE_true_df['p_value']<p_val_thresh)]
        u = DE_pred_df[DE_pred_df['target']==p]
        if t.empty or u.empty:
            matches[p] = np.nan; continue
        m = pd.merge(t[['feature','fold_change']], u[['feature','fold_change']], on='feature', suffixes=('_t','_p'))
        if m.empty:
            matches[p] = np.nan; continue
        matches[p] = float(((m['fold_change_t']<1)==(m['fold_change_p']<1)).mean())
    return matches
...
def compute_DE_for_truth_and_pred(
    ...
    start = time.time()
    # Real DE
    DE_true_fc, DE_true_pval, DE_true_pval_fc, DE_true_sig, DE_true_df = parallel_compute_de(
        adata_real_ct, control_pert, pert_col, outdir, 'real')
    tools_logger.info(f"True DE in {time.time()-start:.2f}s")
    # Pred DE
    start = time.time()
    adata_pred_ct.var.index = adata_real_ct.var.index
    DE_pred_fc, DE_pred_pval, DE_pred_pval_fc, DE_pred_sig, DE_pred_df = parallel_compute_de(
            adata_pred_ct, control_pert, pert_col, outdir, 'pred')
    tools_logger.info(f"Pred DE in {time.time()-start:.2f}s")
    return DE_true_fc, DE_pred_fc, DE_true_pval, DE_pred_pval, DE_true_pval_fc, DE_pred_pval_fc, DE_true_sig, DE_pred_sig, DE_true_df, DE_pred_df


def compute_DE_pca(adata_pred: ad.AnnData, gene_names: Sequence, pert_col: str, control_pert: str, k: int = 50, transform=None) -> pd.DataFrame:
    """DE on PCA-decoded expression."""
    if transform is None:
        raise ValueError("PCA transform required")
    dec = transform.decode(adata_pred.X).cpu().numpy()
    ad = ad.AnnData(X=dec, obs=adata_pred.obs, var=pd.DataFrame(index=gene_names))
    sc.tl.rank_genes_groups(ad, groupby=pert_col, reference=control_pert, rankby_abs=True, n_genes=k)
    return pd.DataFrame(ad.uns['rank_genes_groups']['names']).T


def compute_downstream_DE_metrics(target: str, pred_df: pd.DataFrame, true_df: pd.DataFrame, p_val_threshold: float) -> Dict:
    true_sub = true_df[(true_df['target']==target)&(true_df['p_value']<p_val_threshold)]
    pred_sub = pred_df[pred_df['target']==target]
    genes = true_sub['feature'].tolist()
    res = {'target': target, 'significant_genes_count': len(genes), 'spearman': np.nan, 'pr_auc': np.nan, 'roc_auc': np.nan}
    if not genes:
        return res
    merged = pd.merge(true_sub[['feature','fold_change']], pred_sub[['feature','fold_change']], on='feature', suffixes=('_t','_p'))
    if len(merged)>1:
        res['spearman'] = float(spearmanr(merged['fold_change_t'], merged['fold_change_p'])[0])
    lab = true_sub.assign(label=(true_sub['p_value']<p_val_threshold).astype(int))
    mc = pd.merge(lab[['feature','label']], pred_sub[['feature','p_value']], on='feature')
...
def compute_mean_perturbation_effect(adata: ad.AnnData, pert_col: str = 'gene', ctrl_pert: str = 'non-targeting') -> pd.DataFrame:
    df = adata.to_df()
...
def compute_perturbation_id_score(adata_pred: ad.AnnData, adata_real: ad.AnnData, pert_col: str = 'gene', ctrl_pert: str = 'non-targeting') -> float:
    ...
    preds = []
    for p in perts:
        idx = np.argmax(cosine_similarity(me_r.loc[p].values.reshape(1,-1), me_p.values))
        preds.append(perts[idx])
    return float((np.array(preds)==perts).mean())


def compute_perturbation_ranking_score(adata_pred: ad.AnnData, adata_real: ad.AnnData, pert_col: str = 'gene', ctrl_pert: str = 'non-targeting') -> float:
    me_r = compute_mean_perturbation_effect(adata_real, pert_col, ctrl_pert)
    me_p = compute_mean_perturbation_effect(adata_pred, pert_col, ctrl_pert)
    perts = me_r.index.values
    ranks = []
    for p in perts:
        sim = cosine_similarity(me_r.loc[p].values.reshape(1,-1), me_p.values).flatten()
        rank = np.where(np.argsort(sim)[::-1]==np.where(perts==p)[0][0])[0][0]
        ranks.append(rank)
    return float(np.mean(ranks)/len(perts))


def vectorized_de(de_results: pd.DataFrame, control_pert: str, sort_by: str = 'abs_fold_change') -> pd.DataFrame:
